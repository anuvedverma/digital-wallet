...
function compare_outputs {
  ...
  PROJECT_ANSWER_PATH4=${GRADER_ROOT}/temp/paymo_output/output4.txt
  ...
  TEST_ANSWER_PATH4=${GRADER_ROOT}/tests/${test_folder}/paymo_output/output4.txt
  ...
  if [ "${DIFF_RESULT4}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH4} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (output4.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (output4.txt)"
